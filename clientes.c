...
void ler_arquivo_teste(FILE* arquivo, Clientes** lista){
    ...
    while(fgets(linha, sizeof(linha), arquivo) != NULL){
        ...
        if(strcmp(linha, "Cliente:\n") == 0){
            Clientes* novo_cliente = (Clientes*) malloc(sizeof(Clientes));
            if(novo_cliente == NULL){
                printf("Erro ao alocar memória para um novo cliente.\n");
                fclose(arquivo);
                exit(EXIT_FAILURE);
            }
            ...
            if(strcmp(linha, "\tSem pedidos adicionados a esse cliente.\n") != 0){
                novo_cliente->lista = (Lista_Pedidos*) malloc(sizeof(Lista_Pedidos));
                if(novo_cliente->lista == NULL){
                    printf("Erro ao alocar memória para lista de pedidos.\n");
                    fclose(arquivo);
                    exit(EXIT_FAILURE);
                }
                ...
                while(strncmp(linha, "\tId:", 4) == 0){    
                    ...
                    if(novo_pedido == NULL){
                        printf("Erro ao alocar memória para um novo pedido.\n");
                        fclose(arquivo);
                        exit(EXIT_FAILURE);
                    }
                    ...
                    while(fgets(linha, sizeof(linha), arquivo) != NULL){
                        if(linha[0] == '\n' || strncmp(linha, "\t\t", 2) != 0){
                            ...
                            printf("entrou na lista de itens.\n");
                            ...
                            if(novo_item->item == NULL){
                                printf("erro ao alocar memória para estrutura item.\n");
                                fclose(arquivo);
                                exit(EXIT_FAILURE);
                            }
                    ...
                    if(novo_cliente->lista->pedido == NULL){
                        printf("add pedido ao primeiro no\n");
                        novo_cliente->lista->pedido = novo_pedido;
                        novo_cliente->lista->prox = NULL;
                    }
                    else{
                        printf("tentou add pedido ao meio\n");
                        Lista_Pedidos* temporaria = novo_cliente->lista;
                        while(temporaria->prox != NULL){
                            temporaria = temporaria->prox;
                        }

                        Lista_Pedidos* novo_no_pedido = (Lista_Pedidos*)malloc(sizeof(Lista_Pedidos));
                        if (novo_no_pedido == NULL) {
                            printf("Erro ao alocar memória para novo nó de pedido.\n");
                            fclose(arquivo);
                            exit(EXIT_FAILURE);
                        }
            ...
            if(*lista == NULL){
                ...
                *lista = novo_cliente; // Se a lista estiver vazia, o novo cliente será o primeiro
                
            } else {
            // Procurando pelo último cliente na lista
                Clientes *temp = *lista;
